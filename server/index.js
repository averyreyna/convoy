...
// Chart type mapping: Convoy type → Datawrapper visualization type
...
/**
 * Check whether a Datawrapper API token is configured.
 */
app.get('/api/datawrapper/status', (_req, res) => {
  const hasToken = !!process.env.DATAWRAPPER_API_TOKEN;
  res.json({ configured: hasToken });
});

/**
 * Export data to Datawrapper: create chart, upload CSV, configure, optionally publish.
 *
 * Body: { title, chartType, csvData, metadata?, publish? }
 * - chartType can be a Convoy type ("bar") or a Datawrapper type ("d3-bars")
 * - metadata is passed directly as the Datawrapper chart metadata patch
 */
app.post('/api/datawrapper/export', async (req, res) => {
  ...
  // Resolve chart type: accept either Convoy names or native Datawrapper names
  const dwChartType =
  ...
  try {
    ...
    // Step 3: Optionally configure metadata (description, source, colors, etc.)
    if (metadata && Object.keys(metadata).length > 0) {
      ...
      if (!patchRes.ok) {
        // Non-fatal — chart still exists with data
        console.warn(
